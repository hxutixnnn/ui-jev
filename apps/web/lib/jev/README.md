...
- `candidates.ts`: playground-owned prompt-derived values and atomic candidates.
- `direct-evaluator.ts`: runs the composer's Choice evaluations through the TypeSafe SDK directly — no Gateway key needed.
- `safety.ts`: fail-closed spam/PII/unsafe pre-check.
