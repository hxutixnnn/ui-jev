 ...
 */
export function buildCandidates(prompt: string): {
  ...
  numbers.slice(0, 3).forEach((n, i) => {
    ...
    add(`metric_${i}`, `Metric: ${n.label}, ${n.value}. Request-extracted data.`,
      "Metric", props, `data:metric_${n.label}`);
  });
  if (numbers.length >= 2) {
  ...
  progress.forEach((p, i) => {
    add(`progress_${i}`, `Progress: ${p.label}, ${p.value}%. Request-extracted data.`,
      "Progress", { label: p.label, value: p.value, max: 100 }, `data:progress_${p.label}`);
  });

  // Table rows from request list items.
  const first = sentences(prompt)[0] ?? prompt;
  const items = first.split(/[,;]|\s+and\s+/i)
    .map((s) => s.trim().replace(/^[^:;]{1,30}:\s*/, ""))
    .filter((s) => s.length > 0)
    .slice(0, 8);
  if (items.length >= 2) {
